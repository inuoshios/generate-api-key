...
// func generateUUIDv5(options GenerateKeyOptions) (any, error) {
// 	if options.Length != 0 {
// 		return nil, fmt.Errorf("length is not supported for uuidv5 method")
// 	}
// 	if options.Pool != "" {
// 		return nil, fmt.Errorf("pool is not supported for uuidv5 method")
// 	}
// 	if options.Prefix == "" {
// 		return nil, fmt.Errorf("prefix is required for uuidv5 method")
// 	}

// 	if options.Batch > 1 {
// 		var batchResults []string
// 		for i := uint32(0); i < options.Batch; i++ {
// 			uuid := uuid.NewSHA1(uuid.New(), []byte(options.Prefix)).String()
// 			var result string
// 			result = options.Prefix
// 			result += uuid
// 			if options.Dashes {
